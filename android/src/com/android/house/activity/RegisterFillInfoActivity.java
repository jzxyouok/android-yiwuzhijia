...
import com.android.house.businessresponse.RegistBusinessResponse;
...
import com.android.house.view.DealDialog;
import com.external.eventbus.EventBus;
import com.external.maxwin.view.XListView;
import com.external.maxwin.view.XListView.IXListViewListener;
import com.funmi.house.R;

public class RegisterFillInfoActivity extends BaseActivity implements OnClickListener,IXListViewListener{
	 ...
	 Handler handler=new Handler()
	{
		...
		public void handleMessage(Message msg) {
			...
			case AppConstants.JSON_VALID_OK:
				{
					...
					Toast.makeText(RegisterFillInfoActivity.this, "验证码获取成功，请留意您的短信哦，亲~", Toast.LENGTH_SHORT).show();
				}
				break;
			case AppConstants.JSON_VALID_ERR:
				{
					refreshReg=true;
					Log.d("mao","获取验证码失败");
					Toast.makeText(RegisterFillInfoActivity.this, msg.obj.toString(), Toast.LENGTH_SHORT).show();
				}
				break;
			case AppConstants.LOGIN_OK:
				{
					...
					intent.putExtra("phone", registInfo.getPhone());
			...
			case AppConstants.JSON_NETWORK_ERROR:
				{
					Toast.makeText(RegisterFillInfoActivity.this, "网络出错", Toast.LENGTH_SHORT).show();
				}
				break;
			default:
				break;
	...
	private void initView(){
		//header = LayoutInflater.from(RegisterFillInfoActivity.this).inflate(R.layout.layout_register_fill_info, null);
		/*xlistView = (XListView)findViewById(R.id.register_fill_info_list);
	...
	public void onClick(View v) {
		...
		case R.id.register_btn:
			...
			if(phoneStr.equals("")){
				...
				}else if(certifyCodeStr.equals("")){
					Toast.makeText(getApplicationContext(), "请输入验证码", Toast.LENGTH_SHORT).show();
				}else if(certifyCodeStr.length()<5){
					Toast.makeText(getApplicationContext(), "请输入6位验证码", Toast.LENGTH_SHORT).show();
				}else if(psdStr.equals("")){
				Toast.makeText(getApplicationContext(), "请输入密码", Toast.LENGTH_SHORT).show();
			}
			else if(psdStr.length()<6){
		...
		case R.id.activity_register_send_cerifycode:
			...
			if(count == 60){
				...
				sendCodeBtn.setBackgroundResource(R.drawable.btn_shape_light_gray);
